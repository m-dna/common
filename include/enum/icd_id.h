...
enum class IcdId : uint16_t {
  ...
  PCAM_VIDEO_DATA = 0x001E,       // PCA 비디오 데이터 (pcam_video_data)
  HOMING_START_COMMAND = 0x001F,  // homing_start_command
  HOMING_START_RES = 0x0020,      // homing_start_res
  HOMING_END_COMMAND = 0x0021,    // homing_end_command
  HOMING_END_RES = 0x0022,        // homing_end_res
  TELEMETRY_DATA = 0x0023,        // telemetry_data
  HILS_ROTATE_COMMAND = 0x0024,   // hils_rotate_command
  IMU_ATTITUDE_FEEDBACK = 0x0025, // imu_attitude_feedback
  IMU_CALIBRATION_DATA = 0x0026,  // imu_calibration_data
  // 아래는 테스트용 ICD ID입니다. 실제 운용에서는 사용X.
  COMM_RELIABLE_TEST =
